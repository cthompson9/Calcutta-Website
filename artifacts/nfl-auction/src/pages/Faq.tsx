      ...
      <header className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-xs font-mono uppercase tracking-wider text-muted-foreground md:text-sm md:tracking-widest">
            Common questions and setup help
          </p>
        </div>
      </header>

      <section aria-labelledby="mcp-api-faq-title" className="space-y-4">
        <div className="flex items-center gap-2 border-b border-border pb-2">
          <h2 id="mcp-api-faq-title" className="text-sm font-mono font-bold uppercase tracking-widest">
        ...
        <details open className="group border border-border bg-background" data-testid="details-mcp-api-faq">
          ...
          <div className="space-y-4 border-t border-border px-4 py-4 text-sm leading-relaxed text-muted-foreground">
            <p>
              ...
              <code className="border border-border bg-muted px-1.5 py-0.5 font-mono text-xs text-foreground">/api/mcp</code>.
              The endpoint uses stateless Streamable HTTP and accepts{" "}
              <code className="border border-border bg-muted px-1.5 py-0.5 font-mono text-xs text-foreground">POST</code>{" "}
              requests.
            </p>
            ...
            <div className="space-y-2 border border-border bg-muted/40 p-3">
              <p className="font-mono text-xs font-bold uppercase tracking-wide text-foreground">Claude custom connector</p>
              <p className="text-xs">
              ...
              <div className="mt-4 space-y-4">
                {claudeSetupSteps.map((step, index) => (
                  <figure key={step.image} className="overflow-hidden border border-border bg-background" data-testid={`claude-setup-step-${index + 1}`}>
                    <div className="grid gap-3 p-3 md:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] md:items-start">
                      <figcaption className="text-xs leading-relaxed">
                        <p className="font-mono font-bold uppercase tracking-wide text-foreground">Step {index + 1}: {step.title}</p>
                      ...
                      <img src={`${import.meta.env.BASE_URL}claude-setup/${step.image}`} alt={step.alt} loading="lazy" className="h-auto w-full border border-border object-contain" />
                    </div>
                  </figure>
                ))}
              </div>
        ...
        <details className="group border border-border bg-background" data-testid="details-mcp-prompts-faq">
          ...
          <div className="space-y-4 border-t border-border px-4 py-4 text-sm leading-relaxed text-muted-foreground">
            <p>These are good starting prompts. Include a season when you want a historical answer instead of the active season:</p>
            <ul className="list-disc space-y-2 pl-5 text-xs">
              <li><code className="font-mono text-foreground">Show the current owners, auction cost, wins, and realized return for the Seattle Seahawks in 2026.</code></li>
              <li><code className="font-mono text-foreground">Compare signed realized returns by consortium for the 2025 and 2026 Calcuttas.</code></li>
              <li><code className="font-mono text-foreground">Show the mark-to-market value and return for the 49ers in the current season.</code></li>
              <li><code className="font-mono text-foreground">Check trade 123 and summarize its status and audit history.</code></li>
              <li><code className="font-mono text-foreground">Prepare a 25% trade of the Seahawks from Alex to Jordan for $100, but do not approve it.</code></li>
            </ul>
            <p className="border-l-2 border-primary pl-3 text-xs font-medium text-foreground">
              Creating a trade leaves it pending for review. Other changes may be restricted depending on the action.
            </p>
          </div>
        </details>
      </section>

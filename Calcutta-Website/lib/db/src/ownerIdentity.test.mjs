...
import { resolveHistoricalOwnerIdentity } from "./historicalOwnerIdentity.ts";
...
test("owner identity decision file covers all 73 source records", async () => {
  ...
  assert.equal(report.personCount, 33);
  ...
  for (const document of documents) {
    for (const owner of document.owners) {
      ...
      assert.equal(
        mapping.records.find((candidate) => candidate.record === record)?.person,
        resolveHistoricalOwnerIdentity(document.edition, owner.label).person,
        `${record} compatibility mapping must match the exact edition/label decision`,
      );
    }
  }
});

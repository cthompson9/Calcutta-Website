export const calendarProjectionParticipantIdentityMigration = {
  version: "0038_calendar_projection_participant_identity",
  sql: `
    ...
    begin
      if tg_table_name = 'calcutta_entries' then
        ...
        and (new.slot_id is distinct from old.slot_id or new.mtm_snapshot_id is distinct from old.mtm_snapshot_id) then
        ...
        and (new.status is distinct from old.status or new.pool_id is distinct from old.pool_id)
        and exists (select 1 from calendar_projection_snapshots where mtm_snapshot_id=old.id) then

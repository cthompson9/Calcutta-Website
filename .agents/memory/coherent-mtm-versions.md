...
Displayed Results, normalized HTTP valuation, and MCP current team/owner valuation resolve dollars and associated metadata from the promoted coherent version only. Live Tracker history includes every distinct current or superseded coherent version source snapshot, ordered by its exact capture timestamp; it must not collapse multiple promoted pulls into one weekly point. Pipeline snapshots and projections remain evidence unless explicitly tied to a promoted version’s source snapshot; unavailable current versions fail closed.
...
**How to apply:** Route future actuals-refresh promotion and every Results/MCP valuation consumer through the shared current-version promotion and resolver boundary. Build chart history from promoted source snapshots on a continuous time axis, using week names only as tick labels. Keep the commissioner button on the complete ordered refresh path. Do not duplicate team values in version rows or mutate referenced snapshot evidence.
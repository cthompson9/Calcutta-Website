...
**How to apply:** The official 2026 acceptance band is five percentage points and must be read from the shared season configuration by both simulation and publication validation. Preserve deterministic coordinate order, redundant-inventory anchor handling, support checks, and final all-target residual validation. Optimization equivalence tests must use this tolerance-band solver as the legacy reference, not the superseded exact-point solver. Select path count from matching-state ESS plus entry and conditional EV stability against a larger reference, not runtime alone.